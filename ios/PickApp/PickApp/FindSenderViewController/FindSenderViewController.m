...
@interface FindSenderViewController ()<UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate>
{
    void * _KVOContext;
...
@implementation FindSenderViewController
{
...
//     tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 264, self.view.frame.size.width, self.view.frame.size.height - 44)];
   ...
   //  [self.view addSubview:tableView];
    ...
    menuviewc.sendslctline.hidden=FALSE;
    ...
    UITapGestureRecognizer *addsnder = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(addsnderActn)];
    addsnder.numberOfTapsRequired = 1;
    [self.addSender addGestureRecognizer:addsnder];
    self.addSender.userInteractionEnabled = YES;
...
- (void) webserviceCallForSenderList
{
    ...
    [dict setValue:@"getSender" forKey:@"id"];
