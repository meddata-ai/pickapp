...
- (void)viewDidLoad {
    ...
    UITapGestureRecognizer *finddelver = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(finddelverActn)];
    back.numberOfTapsRequired = 1;
    [self.finddelver addGestureRecognizer:finddelver];
    self.finddelver.userInteractionEnabled = YES;
...
//     tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 264, self.view.frame.size.width, self.view.frame.size.height - 44)];
   ...
   [self.bottomMenu addSubview:menuviewc.view];
    ...
    self.finddelver.layer.cornerRadius=20.0f;
    self.finddelver.layer.borderWidth=2.0f;

    self.finddelver.layer.borderColor=[UIColor whiteColor].CGColor;
    self.finddelver.clipsToBounds=YES;
    ...
    [self.sbmtbtn addGestureRecognizer:submtbtn];
    self.sbmtbtn.userInteractionEnabled = YES;
 ...
 cancelButton.tag=1;
  ...
  self.dateField.inputAccessoryView = toolBar;
    ...
    //self.phoneField.inputAccessoryView = toolBar1;
...
- (IBAction)submitAct:(id)sender {
   ...
   else if (self.emailField == nil || [self.emailField.text isEqualToString:@""])
   {
       ...
       [self webserviceCallForAddSender];
   }
}
/*
...
*/
- (void) webserviceCallForAddSender
{
...
//    senderdata= [[NSMutableArray alloc]init];
    ...
    [dict setValue:@"addSender" forKey:@"id"];
...
//https://deckwebtech.com/pickapp_demo/addingData.php?id=addSender&apiKey=YavK2cw5ZoBEqTf6W0HCG1OiXx9d7Jnu&departure=&destination=&date=2021-09-12&fee=&comment=&phone=&email=&viewEmail=True&viewPhone=false
